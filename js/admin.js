...
import { statusLabel, statusBadgeClass, deleteRoom as apiDeleteRoom, deleteRooms as apiDeleteRooms } from './rooms.js?v=2';
...
export async function deleteRooms(ids) {
  return await apiDeleteRooms(ids);
}
        ...
        <tbody>
          ${users.map(u => `
            <tr>
              ...
              <td>${new Date(u.created_at).toLocaleDateString('ar-SA-u-ca-gregory-nu-latn')}</td>
              <td>
...
export function renderRoomsTable(rooms, container, onDelete, onBulkDelete) {
  if (!rooms.length) {
    container.innerHTML = '<p class="text-muted text-center" style="padding:40px">لا توجد غرف بعد</p>';
    return;
  }
  ...
  container.innerHTML = `
    <div class="flex justify-between items-center p-16 bg-card-dark" style="border-bottom: 1px solid var(--border)">
        <div class="flex items-center gap-12">
            <input type="checkbox" id="select-all-rooms" style="width:18px;height:18px">
            <span class="text-sm text-muted">تحديد الكل</span>
        </div>
        <button id="bulk-delete-rooms-btn" class="btn btn-sm btn-danger hidden">حذف المحدد</button>
    </div>
    <div class="table-wrapper">
      <table>
        <thead><tr>
          <th style="width:40px"></th><th>ID</th><th>المنشئ</th><th>الشريك</th><th>الحالة</th><th>نسبة التوافق</th><th>التاريخ</th><th>حذف</th>
        </tr></thead>
        <tbody>
          ${rooms.map(r => `
            <tr>
              <td><input type="checkbox" class="room-checkbox" data-id="${r.id}" style="width:16px;height:16px"></td>
              <td style="font-size:0.8rem;color:var(--text-muted)">${r.id}</td>
              ...
              <td>${new Date(r.created_at).toLocaleDateString('ar-SA-u-ca-gregory-nu-latn')}</td>
